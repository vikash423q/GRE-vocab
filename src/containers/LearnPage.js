...
const LearnPage = (props) => {
    ...
    React.useEffect(() => { dispatch({ type: 'LOAD_LEARN_SESSION' }) }, []);
    ...
    return (
        <Grid container className={classes.page} direction="column" justify="flex-start" align="flex-start">
            <Grid item container justify="center" align="center">
                <Grid item>
                    <IconButton onClick={() => dispatch({ type: 'CHANGE_LEARN_SESSION', payload: state.session_id - 1 })}><ArrowBackOutlinedIcon /></IconButton>
                </Grid>
                ...
                <Grid item>
                    <IconButton onClick={() => dispatch({ type: 'CHANGE_LEARN_SESSION', payload: state.session_id + 1 })}><ArrowForwardOutlinedIcon /></IconButton>
                </Grid>
            </Grid>
            <Grid item style={{ paddingTop: 12 }}>
                <Typography variant="body2">{"**scores will appear on completion**"}</Typography>
            </Grid>
            <Container className={classes.gap} style={{ height: 16 }} />
            <Grid item container direction="column">
                {state.session.map((item, idx) => <Grid item className={classes.grid} container direction="column" justify="flex-start" align="flex-start">
                    <Grid container direction="column" item>
                        <Grid item>
                            <Typography variant="h5">{(idx + 1) + ". " + capitalize(item.word)}</Typography>
                        </Grid>
                        <Grid item style={{ paddingLeft: 16 }}>
                            <Typography variant="subtitle1" style={{ paddingTop: 4, paddingBottom: 4 }}>{item.definition}</Typography>
                        </Grid>
                        <Container className={classes.gap} style={{ height: 8 }} />
                        {item.dictionary.pronunciation ?
                            <Grid item style={{ paddingLeft: 16 }}>
                                <Typography variant="subtitle2"><i>pronunciation:</i>{` ${item.dictionary.pronunciation.all}` || ''}</Typography>
                            </Grid> : null}
                    </Grid>
                    <Container className={classes.gap} />
                    {item.dictionary['results'].slice(0, 3).map(el => {
                        return (<Grid direction="column" style={{ paddingLeft: 16 }} container item key={el.definition}>
                            <Grid item>
                                <Typography variant="body2"><i>definition: </i>{`(${el.partOfSpeech}) ${el.definition}`}</Typography>
                            </Grid>
                            {el.synonyms && el.synonyms.length > 0 ?
                                <Grid item>
                                    <Typography variant="body2"><i>synonym:</i>{` ${el.synonyms.join(', ')}`}</Typography>
                                </Grid> : null}
                            {el.examples ? el.examples.slice(0, 3).map(ex => <Grid key={ex} item>
                                <Typography variant="body2"><i>example: </i>{`" ${ex}"`}</Typography>
                            </Grid>) : null}

                        </Grid>);
                    })}
                    <Divider className={classes.part} />
                </Grid>)}
